...
export const ColorfulOptionsetControl = React.memo(
  ({
    rawOptions,
    selectedKey,
    onChange,
    ...
    defaultValue,
    config,
  }: IColorfulOptionsetProperties): JSX.Element => {
    console.log("%cEntered control", "color:red");
    const allOptions = [
      { Label: "--Select--", Value: -1, Color: "transparent" },
      ...rawOptions,
    ];
    ...
    const _renderOption = (
      ...
      const icon =
        (config.jsonConfig && option?.key
          ? config.jsonConfig[option?.key]?.icon
          : config.defaultIconName) ?? config.defaultIconName;
      const defaultColor = option?.data?.color || "#ffffff";
      const color =
        (config.jsonConfig && option?.key
          ? config.jsonConfig[option?.key]?.color
